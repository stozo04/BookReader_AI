...
@Component({
    ...
    <div class="flex h-screen font-sans bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-200">
      ...
      <aside
        ...
        <div class="p-4 border-t border-gray-200 dark:border-gray-700">
          <button
            (click)="openSettings()"
            class="w-full flex items-center justify-start px-4 py-2 text-sm font-semibold rounded-md transition-colors text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700">
      ...
      <div class="flex-1 flex flex-col transition-all duration-300 ease-in-out" [class.ml-80]="isMenuOpen()">
        ...
        <main class="flex-1 p-6 md:p-10 lg:p-12 overflow-y-hidden">
          @if (progressService.currentChapter(); as chapter) {
            <div class="max-w-3xl mx-auto h-full flex flex-col">
              ...
              <div #pageContainer
                   class="prose dark:prose-invert max-w-none text-lg leading-relaxed flex-grow overflow-hidden relative"
                   [style.font-size.%]="progressService.fontSize()">
                @if (currentPageContent()) {
                  <!-- Removed @key (Angular 18+). Use a stable attribute to force re-rendering -->
                  <div
                    class="whitespace-pre-wrap h-full absolute inset-0"
                    [class.animate-slide-in-from-right]="navigationDirection() === 'next'"
                    [class.animate-slide-in-from-left]="navigationDirection() === 'prev'"
                    [class.animate-fade-in]="navigationDirection() === 'none' || navigationDirection() === 'initial'"
                    (mouseup)="onTextSelection($event)"
                    [attr.data-key]="progressService.currentChapterIndex() + '-' + progressService.currentPageIndex()">
                    {{ currentPageContent() }}
                  </div>
                }
              </div>
        ...
        <footer class="flex items-center justify-between p-4 bg-white dark:bg-gray-800 shadow-inner">
          <button
            (click)="previousPage()"
            [disabled]="isFirstPageOfBook()"
            class="px-4 py-2 font-semibold rounded-lg disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-200 dark:hover:bg-gray-700">
            Previous
          </button>
      ...
      @if (selectedText()) {
        <div
          class="absolute z-40 p-2 bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 flex space-x-2"
          [style.top.px]="selectionRect()?.top - 50"
          [style.left.px]="selectionRect()?.left + (selectionRect()?.width / 2) - 130">
          <button
            (click)="getDefinition(selectedText())"
            [disabled]="!canDefine()"
            class="px-3 py-1 text-sm rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed">
            Define
          </button>
          <button
            (click)="viewCharacter(selectedText())"
            class="px-3 py-1 text-sm rounded-md hover:bg-gray-100 dark:hover:bg-gray-700">
            Character
          </button>
          <button
            (click)="generateScene(selectedText())"
            [disabled]="!canSummarizeOrVisualize()"
            class="px-3 py-1 text-sm rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed">
            Visualize
          </button>
          <button
            (click)="summarizeSelection(selectedText())"
            [disabled]="!canSummarizeOrVisualize()"
            class="px-3 py-1 text-sm rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed">
            Summarize
          </button>
                      ...
                      <div class="flex flex-col md:flex-row gap-6">
                        <div class="flex-shrink-0 md:w-1/3">
                          @if (interactionState().data.imageUrl) {
                            <img
                              [src]="'data:image/jpeg;base64,' + interactionState().data.imageUrl"
                              [alt]="'Image of ' + interactionState().data.characterName"
                        ...
                        <div class="flex items-center justify-between">
                          ...
                          <div class="relative inline-block w-10 mr-2 align-middle select-none transition duration-200 ease-in">
                            <input
                              ...
                              (change)="progressService.toggleTheme()"/>
                        ...
                        <div class="space-y-4">
                          <div class="flex items-center justify-between">
                            ...
                            <div class="flex items-center space-x-4">
                              <button
                                (click)="progressService.decreaseFontSize()"
                                [disabled]="!canDecreaseFontSize()"
                                class="px-3 py-1 text-lg font-bold rounded-md bg-gray-200 dark:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed">-</button>
                              <span class="w-12 text-center">{{ progressService.fontSize() }}%</span>
                              <button
                                (click)="progressService.increaseFontSize()"
  ...
  styles: `
    ...
    .animate-slide-in-from-left { animation: slideInFromLeft 0.3s ease-out forwards; }
...
export class EbookReaderComponent implements AfterViewInit, OnDestroy {
  ...
  navigationDirection = signal<'next' | 'prev' | 'none' | 'initial'>('initial');

  currentPageContent = computed(() => this.pages()[this.progressService.currentPageIndex()] || '');
  isFirstPageOfBook = computed(
    () => this.progressService.currentChapterIndex() === 0 && this.progressService.currentPageIndex() === 0,
  );
  ...
  canDecreaseFontSize = computed(() => this.progressService.fontSize() > 70);
  canIncreaseFontSize = computed(() => this.progressService.fontSize() < 150);

  modalTitle = computed(() => {
    switch (this.interactionState().type) {
      case 'character':
        return `Character Profile: ${this.interactionState().data?.characterName}`;
      case 'summary':
        return `Summary`;
      case 'scene-image':
  ...
  constructor() {
    effect(() => {
      // Repaginate when the current chapter or font size changes.
      this.progressService.currentChapter();
      this.progressService.fontSize(); // Track font size changes
      this.paginate(false);
    });

    effect(() => {
      // Handle jumping to the last page of a chapter.
  ...
  private setupResizeObserver() {
    ...
    if (container) {
      this.resizeObserver = new ResizeObserver(() => this.paginate(true));
      this.resizeObserver.observe(container.nativeElement);
    }
  }

  private paginate(isResize = false) {
    if (isResize) {
    ...
    if (!container || !content || container.clientHeight === 0) {
      this.pages.set([]);
      if (this.progressService.currentPageIndex() !== 0) {
        this.progressService.goToPage(0);
      }
      return;
    }
    ...
    let anchorText = '';
    if (isResize) {
      const oldPageIndex = this.progressService.currentPageIndex();
      const oldPages = this.pages();
      if (oldPages.length > oldPageIndex && oldPages[oldPageIndex]) {
        anchorText = oldPages[oldPageIndex].trim().split(/\s+/).slice(0, 10).join(' ');
      }
    }
    ...
    measurer.style.textAlign = style.textAlign as string;
    ...
    let targetPageIndex = this.progressService.currentPageIndex();

    if (isResize && anchorText && pagesResult.length > 0) {
      const foundIndex = pagesResult.findIndex((p) => p.includes(anchorText));
      if (foundIndex !== -1) {
    ...
    if (targetPageIndex >= pagesResult.length) {
      targetPageIndex = pagesResult.length > 0 ? pagesResult.length - 1 : 0;
    }

    if (this.progressService.currentPageIndex() !== targetPageIndex) {
      this.progressService.goToPage(targetPageIndex);
    }
  }
  ...
  previousPage() {
    const currentPage = this.progressService.currentPageIndex();
    this.navigationDirection.set('prev');
    if (currentPage > 0) {
      this.progressService.goToPage(currentPage - 1);
  ...
  async viewCharacter(name: string) {
    ...
    if (!characterName) {
      this.interactionState.set({
        ...
        data: null,
        loading: false,
        error: `"${name}" is not recognized as a main character.`,
        loadingMessage: '',
      });
      return;
    }
    ...
    this.interactionState.set({
      ...
      loading: true,
      error: null,
      loadingMessage: `Generating profile for ${characterName}...`,
    });
    ...
    try {
      ...
      if (profile?.physicalAppearance) {
        imageUrl = await this.geminiService.generateCharacterImage(
          characterName,
      ...
      this.interactionState.set({
        type: 'character',
        data: { characterName, profile, imageUrl },
        loading: false,
        error: null,
        loadingMessage: '',
      });
  ...
  async summarizeSelection(selectedText: string) {
    ...
    this.interactionState.set({
      ...
      loading: true,
      error: null,
      loadingMessage: 'Generating summary for selection...',
    });
  ...
  async viewChapterSummary(index: number) {
    ...
    this.interactionState.set({
      ...
      loading: true,
      error: null,
      loadingMessage: `Generating summary for "${chapter.title}"...`,
    });
    this.isMenuOpen.set(false);
  ...
  async generateScene(selectedText: string) {
    ...
    this.interactionState.set({
      ...
      loading: true,
      error: null,
      loadingMessage: 'Generating scene visualization...',
    });
